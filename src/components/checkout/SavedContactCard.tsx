...
import { Mail, Pencil, Star, Trash2 } from "lucide-react";
import type { SavedContact } from "@/types/checkout";
      ...
      >
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 text-primary/90">
            <Mail size={14} strokeWidth={1.25} className="shrink-0" />
            {contact.isDefault && (
        ...
        <p className="font-serif text-lg text-white tracking-wide">{contact.fullName}</p>
        <p className="mt-1 text-[11px] text-white/55">{contact.email}</p>
        <p className="mt-3 text-[11px] text-white/45">{contact.phone}</p>
      </button>
